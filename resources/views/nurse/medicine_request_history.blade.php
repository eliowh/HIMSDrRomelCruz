...
@section('title','Medicine Request History')

@section('content')
@php $requests = $requests ?? collect(); $q = $q ?? ''; @endphp
        ...
        <div class="inventory-card">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:12px;">
                ...
                <form method="GET" style="margin-left:auto;display:flex;align-items:center;gap:8px;">
                    <input type="search" name="q" value="{{ $q }}" placeholder="Search requests..." style="padding:8px 10px;border:1px solid #ddd;border-radius:6px; height: 40px" />
                    ...
                    <a href="{{ route('nurse.patients.index') }}" class="action-btn secondary" style="margin-left:8px;text-decoration:none;">Back to Patients</a>
                </form>
            </div>
            ...
            @if($requests->count())
                <div class="table-wrap">
                    <table class="patients-table" style="width:100%;border-collapse:collapse;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td style="padding:10px;border-bottom:1px solid #f2f4f7;">#{{ $request->id }}</td>
                                <td style="padding:10px;border-bottom:1px solid #f2f4f7;">
                                    <div>
                                        <strong>{{ $request->patient_name ?? '-' }}</strong>
                                        @if($request->patient_no)
                                            <br><small style="color:#666;">{{ $request->patient_no }}</small>
                                        @endif
                                    </div>
                                </td>
                                <td style="padding:10px;border-bottom:1px solid #f2f4f7;">
                                    <div>
                                        <strong>{{ $request->generic_name ?: $request->brand_name ?: '-' }}</strong>
                                        @if($request->item_code)
                                            <br><small style="color:#666;">{{ $request->item_code }}</small>
                                        @endif
                                        @if($request->generic_name && $request->brand_name)
                                            <br><small style="color:#666;">{{ $request->brand_name }}</small>
                                        @endif
                                    </div>
                                </td>
                                <td style="padding:10px;border-bottom:1px solid #f2f4f7;text-align:center;">{{ $request->quantity }}</td>
                                <td style="padding:10px;border-bottom:1px solid #f2f4f7;text-align:right;">
                                    ₱{{ number_format($request->total_price ?? 0, 2) }}
                                </td>
                                ...
                                <td style="padding:10px;border-bottom:1px solid #f2f4f7;">
                                    {{ $request->requested_at ? $request->requested_at->format('M d, Y h:i A') : '-' }}
                                </td>
                                <td style="padding:10px;border-bottom:1px solid #f2f4f7;">
                                    @if($request->status === 'dispensed' && $request->dispensed_at)
                                        {{ $request->dispensed_at->format('M d, Y h:i A') }}
                                        @if($request->dispensedBy)
                                            <br><small style="color:#666;">by {{ $request->dispensedBy->name }}</small>
                                        @endif
                                    @elseif($request->status === 'cancelled' && $request->cancelled_at)
                                        {{ $request->cancelled_at->format('M d, Y h:i A') }}
                                        @if($request->cancelledBy)
                                            <br><small style="color:#666;">by {{ $request->cancelledBy->name }}</small>
                                        @endif
                                    @else
                                        -
                                    @endif
                                ...
                                <td style="padding:10px;border-bottom:1px solid #f2f4f7;text-align:center;">
                                    <button type="button" class="btn view-btn" onclick="viewRequestDetails({{ $request->id }})">View Details</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                ...
                @if($requests->hasPages())
                    <div class="pagination-wrapper" style="margin-top:20px;">
                        {{ $requests->appends(request()->query())->links('components.custom-pagination') }}
                    </div>
                @endif
            @else
