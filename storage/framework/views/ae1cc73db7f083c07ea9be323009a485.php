            ...
            <div class="pharmacy-card">
                <?php if($requests->count() > 0): ?>
                    <table class="orders-table">
                        <thead>
                            <tr>
                                <th onclick="sortTable('id')" class="sortable <?php echo e($sort === 'id' ? 'sorted-' . $direction : ''); ?>">
                                    Request ID
                                    <i class="fas fa-sort <?php echo e($sort === 'id' ? ($direction === 'asc' ? 'fa-sort-up' : 'fa-sort-down') : ''); ?>"></i>
                                </th>
                                <th onclick="sortTable('patient_name')" class="sortable <?php echo e($sort === 'patient_name' ? 'sorted-' . $direction : ''); ?>">
                                    Patient
                                    <i class="fas fa-sort <?php echo e($sort === 'patient_name' ? ($direction === 'asc' ? 'fa-sort-up' : 'fa-sort-down') : ''); ?>"></i>
                                </th>
                                <th>Item Code</th>
                                <th onclick="sortTable('generic_name')" class="sortable <?php echo e($sort === 'generic_name' ? 'sorted-' . $direction : ''); ?>">
                                    Medicine
                                    <i class="fas fa-sort <?php echo e($sort === 'generic_name' ? ($direction === 'asc' ? 'fa-sort-up' : 'fa-sort-down') : ''); ?>"></i>
                                </th>
                                <th onclick="sortTable('quantity')" class="sortable <?php echo e($sort === 'quantity' ? 'sorted-' . $direction : ''); ?>">
                                    Qty
                                    <i class="fas fa-sort <?php echo e($sort === 'quantity' ? ($direction === 'asc' ? 'fa-sort-up' : 'fa-sort-down') : ''); ?>"></i>
                                </th>
                                <th>Unit Price</th>
                                <th>Total Amount</th>
                                <th>Requested By</th>
                                <th onclick="sortTable('requested_at')" class="sortable <?php echo e($sort === 'requested_at' ? 'sorted-' . $direction : ''); ?>">
                                    Requested At
                                    <i class="fas fa-sort <?php echo e($sort === 'requested_at' ? ($direction === 'asc' ? 'fa-sort-up' : 'fa-sort-down') : ''); ?>"></i>
                                </th>
                                <th onclick="sortTable('status')" class="sortable <?php echo e($sort === 'status' ? 'sorted-' . $direction : ''); ?>">
                                    Status
                                    <i class="fas fa-sort <?php echo e($sort === 'status' ? ($direction === 'asc' ? 'fa-sort-up' : 'fa-sort-down') : ''); ?>"></i>
                                </th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo e($request->patient_name ?? '-'); ?></td>
                                <td><?php echo e($request->item_code ?? '-'); ?></td>
                                <td><?php echo e($request->generic_name ?? $request->brand_name ?? '-'); ?></td>
                                <td><?php echo e($request->quantity); ?></td>
                                <td>₱<?php echo e(number_format($request->unit_price, 2)); ?></td>
                                <td><strong>₱<?php echo e(number_format($request->total_price, 2)); ?></strong></td>
                                <td><?php echo e(optional($request->requestedBy)->name); ?></td>
                                <td><?php echo e($request->requested_at ? $request->requested_at->format('M d, Y h:i A') : '-'); ?></td>
                                <td><?php echo e(ucfirst(str_replace('_', ' ', $request->status))); ?></td>
                                <td>
                                    <div class="action-dropdown" id="action-dropdown-container-<?php echo e($request->id); ?>">
                                        <button class="action-btn" onclick="toggleDropdown(<?php echo e($request->id); ?>)">
                                            <i class="fas fa-ellipsis-v"></i>
                                        </button>
                                        <div class="dropdown-content" id="dropdown-<?php echo e($request->id); ?>">
                                            <?php if($request->status === 'pending'): ?>
                                                <a href="#" onclick="dispenseRequest(<?php echo e($request->id); ?>)" class="dispense-action">
                                                    <i class="fas fa-pills"></i> Dispense
                                                </a>
                                                <a href="#" onclick="cancelRequest(<?php echo e($request->id); ?>)" class="cancel-action">
                                                    <i class="fas fa-times"></i> Cancel
                                                </a>
                                            <?php else: ?>
                                                <a href="#" onclick="viewRequest(<?php echo e($request->id); ?>)">
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.custom-pagination','data' => ['paginator' => $requests]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
            ...
            <?php if(isset($completedRequests) && $completedRequests->count()): ?>
            <div class="pharmacy-card" style="margin-top:32px;">
                <h3 style="margin-top:0;margin-bottom:16px;display:flex;align-items:center;gap:10px;">
                    ...
                    <span style="background:#eef5ef;color:#2E7D32;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;"><?php echo e($completedRequests->count()); ?></span>
                </h3>
                <table class="orders-table">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <?php $__currentLoopData = $completedRequests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            <td>#<?php echo e($c->id); ?></td>
                            <td><?php echo e($c->patient_name); ?></td>
                            <td><?php echo e($c->generic_name ?? $c->brand_name ?? '-'); ?></td>
                            <td><?php echo e($c->quantity); ?></td>
                            <td>₱<?php echo e(number_format($c->total_price,2)); ?></td>
                            <td><?php echo e(ucfirst(str_replace('_',' ',$c->status))); ?></td>
                            <td>
                                <?php if($c->dispensed_at): ?>
                                    <?php echo e($c->dispensed_at->format('M d, Y h:i A')); ?>

                                <?php elseif($c->completed_at): ?>
                                    <?php echo e($c->completed_at->format('M d, Y h:i A')); ?>
                            ...
                            <td>
                                <?php if($c->dispensedBy): ?>
                                    <?php echo e($c->dispensedBy->name); ?>

                                <?php elseif($c->requestedBy): ?>
                                    <?php echo e($c->requestedBy->name); ?>
                            ...
                            <td>
                                <button class="action-btn" onclick="viewRequest(<?php echo e($c->id); ?>)" title="View"><i class="fas fa-eye"></i></button>
                            </td>
                        </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </tbody>
                </table>
                <p style="margin-top:12px;font-size:12px;color:#666;">Showing latest <?php echo e($completedRequests->count()); ?> processed requests (dispensed or completed). Use filters above for full history.</p>
            </div>
            <?php endif; ?>
        </main>
    </div>
    ...
    <?php echo $__env->make('pharmacy.notification-system', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    ...
    <script>
        ...
        function sortTable(column) {
            ...
            const currentDirection = '<?php echo e($direction); ?>';
...
</html><?php /**PATH D:\xampp\htdocs\DrRomelCruzHP\resources\views\pharmacy\pharmacy_requests.blade.php ENDPATH**/ ?>